...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    Console.print("Synchronous Response:")
    val runtimeConst = StdIn.readLine() //Notice here, val is created at runtime, yet remains immutable
    //That said, we don't WANT to just run readLine() synchronously. Readline is a non-pure function! I don't like that stuff!
    Console.println(runtimeConst)
    ...
    val echoToConsole: IO[Unit] = safeRuntimeConst.map(input => Console.println(input))
   ...
   // echoToConsole.unsafeRunSync() //Synchronously and unsafely run an IO.
    ...
    //Wait a minute, it still didn't do anything?

    //Of course not, it's running asynchronously in its own thread pool!
  }
